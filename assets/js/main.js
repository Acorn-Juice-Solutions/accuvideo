(function () {
  ...
  const i18n = {
    en: {
      ...
      'editions.basic.f6': '<strong>Optional NVIDIA CUDA acceleration</strong> — if a compatible GPU is detected, Basic offers to enable GPU mode and downloads a bundled DLL pack on demand',
      ...
      'editions.pro.f8': '<strong>NVIDIA CUDA build</strong> — separate <strong>.zip (&gt;2 GB)</strong> hosted externally, with SHA-256 hash, linked from each GitHub release',
      ...
      'req.sub': 'AccuVideo Basic runs on commodity laptops. Pro adds visual ingestion (Florence-2). Both editions now support optional NVIDIA CUDA acceleration on Windows — runs on CPU by default, opt into GPU if a compatible card is detected.',
      ...
      'req.min.gpu': 'GPU: integrated is enough. <strong>Optional CUDA acceleration:</strong> if an NVIDIA GPU is detected, Basic offers to enable GPU mode and downloads a small bundled DLL pack (.zip) on demand.',
      ...
      'req.rec.gpu': 'GPU: optional but strongly recommended for visual indexing — NVIDIA GPU with ≥6 GB VRAM and CUDA support. The <strong>CUDA build of Pro</strong> ships as a separate <strong>.zip (&gt;2 GB)</strong> hosted externally; the download link and SHA-256 hash are published next to each GitHub release.',
    ...
    es: {
      ...
      'editions.basic.f6': '<strong>Aceleración NVIDIA CUDA opcional</strong> — si se detecta una GPU compatible, Basic ofrece activar el modo GPU y descarga bajo demanda un pack de DLLs empaquetadas',
      ...
      'editions.pro.f8': '<strong>Build NVIDIA CUDA</strong> — <strong>.zip aparte (&gt;2 GB)</strong> alojado externamente, con hash SHA-256, enlazado desde cada release de GitHub',
      ...
      'req.sub': 'AccuVideo Basic corre en portátiles normales. Pro añade ingesta visual (Florence-2). Ambas ediciones ya soportan aceleración NVIDIA CUDA opcional en Windows — funciona en CPU por defecto, activa la GPU si se detecta una tarjeta compatible.',
      'req.min.title': 'Basic — audio + texto en pantalla',
      'req.min.cpu': 'CPU: Intel i5 (8.ª gen) / Ryzen 5 2600+ / Apple Silicon M1',
      'req.min.ram': 'RAM: 8 GB',
      'req.min.gpu': 'GPU: con la integrada basta. <strong>Aceleración CUDA opcional:</strong> si se detecta una GPU NVIDIA, Basic ofrece activar el modo GPU y descarga bajo demanda un pequeño pack de DLLs (.zip).',
      ...
      'req.rec.gpu': 'GPU: opcional pero muy recomendada para la indexación visual — GPU NVIDIA con ≥6 GB VRAM y soporte CUDA. El <strong>build CUDA de Pro</strong> se distribuye como <strong>.zip aparte (&gt;2 GB)</strong> alojado externamente; el enlace de descarga y su hash SHA-256 se publican junto a cada release de GitHub.',
  ...
  function applyLang(lang) {
    ...
    if (heroVideo) {
      const desired = `assets/videos/demo-${lang}.mp4?v=2`;
